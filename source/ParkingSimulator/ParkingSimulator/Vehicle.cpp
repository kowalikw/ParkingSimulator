...
{
	glm::mat4 rotationMatrix = glm::rotate(glm::mat4(), glm::degrees((float)GetAngle()), glm::vec3(0.0f, 1.0f, 0.0f));
...
PathElement Vehicle::GetTurnCircle(double insideAngle, CircleType circleType, double angleFrom, double angleTo)
{
	...
	return PathElement(centre, radius, angleFrom, angleTo, circleType);
}
