...
Path PathPlanner::CreateAdmissiblePath(vector<Point> points)
{
	Path pathTmp, path;
	...
	{
		glm::vec2 p1(points[i].GetX(), points[i].GetZ());
		glm::vec2 p2(points[i + 1].GetX(), points[i + 1].GetZ());
		glm::vec2 p3(points[i + 2].GetX(), points[i + 2].GetZ());
		...
		glm::vec2 center = GeometryHelper::GetLineIntersectionPoint(P1, P2, P3, P4);
	...
	{
		if (i == 0)
			path.AddElement(PathElement(glm::vec2(points[0].GetX(), points[0].GetZ()), glm::vec2(pathTmp.GetElements()[0].GetCirclePoint(pathTmp.GetElements()[0].GetAngleFrom()))));
		if (i == path.GetElements().size() - 1)
			path.AddElement(PathElement(glm::vec2(pathTmp.GetElements()[pathTmp.GetElements().size() - 1].GetCirclePoint(pathTmp.GetElements()[pathTmp.GetElements().size() - 1].GetAngleTo())), glm::vec2(points[points.size() - 1].GetX(), points[points.size() - 1].GetZ())));
